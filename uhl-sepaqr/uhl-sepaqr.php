<?php

/**
 * UHL SEPA-QR-Code for Woocommerce
 *
 * @package   uhl/sepaqr
 * @author    Bureau fuer Angelegenheiten
 * @license   GPL-2.0+
 * @link      https://github.com/Coernel82/SEPA-QR-for-Woocommerce
 * @copyright 2022 BUFANG
 *
 * @wordpress-plugin
 * Plugin Name:       SEPA-QR-Code for Woocommerce
 * Plugin URI:        https://github.com/Coernel82/SEPA-QR-for-Woocommerce
 * Description:       Adds a SEPA QR code to the Woocommerce Thankyou page and to the confirmation e-mail. The QR code is generated locally due to privacy reasons and is using the IBAN and BIC of the first BACS account.
 * Version:           1.1.0
 * Author:            https://github.com/Coernel82/SEPA-QR-for-Woocommerce
 * Requires at least: 5.0
 * Requires PHP:	  7.0
 * Author URI:        https://github.com/Coernel82/SEPA-QR-for-Woocommerce
 * Text Domain:       mxp-sepa-qr-code-addon-for-woocommerce
 * License:           GPL-2.0+
 * License URI:       http://www.gnu.org/licenses/gpl-2.0.txt
 * Domain Path:       /languages
 */

// If this file is called directly, abort.
if (!defined('ABSPATH')) {
	exit; // Exit if accessed directly
}

/* config parameters */

define('uhl_QUERY_PARAM', 'uhl_uhl_qr'); // name of the query param (only needed if qr link is used)
define('uhl_TRANSIENT_PREFIX', 'uhl_MXPQR_'); // identify our transients or cache entries if the need to be deleted
// activate uhl_USE_TRANSIENTS if longer lifetime beyond current request is desired ,
// otherwise wp-cache is used which will not persist across requests without a persistent caching plugin
// define('uhl_USE_TRANSIENTS',true);
define('uhl_TRANSIENT_LIFETIME', 3 * DAY_IN_SECONDS); //  for transients only, useless used for nonpersistent cache

// Texts and fallback values for iban and bic if none are found
// in a better world, this data would be in fields on an admin page
// if you need one, have a look at https://jeremyhixon.com/tool/wordpress-option-page-generator/

define('uhl_BACS_IBAN', ''); // optional: insert your IBAN as fallback
define('uhl_BACS_BIC', ''); // optional: insert your BIC as fallback
define('uhl_BACS_COMPANY', 'recipient'); // optional: insert your business name as fallback

// Translatable variables

$uhl_THANKYOU_PAGE = 'For a convenient payment scan this qr code!'; // PLAIN Text shown before QR code in TY page
$uhl_THANKYOU_EMAIL = 'For a convenient payment scan this qr code! Some email clients unfortunately will not show Base64 encoded images. Sorry for that!'; // PLAIN Text shown before QR code in Email
$uhl_PURPOSE_PREFIX = 'Order-ID:'; // this string will be prefixed to  the order ID in SEPA purpose Text


/*
define('uhl_THANKYOU_PAGE', 'For a convenient payment scan this qr code!'); // PLAIN Text shown before QR code in TY page
define('uhl_THANKYOU_EMAIL', 'For a convenient payment scan this qr code! Some email clients unfortunately will not show Base64 encoded images. Sorry for that!'); // PLAIN Text shown before QR code in Email
define('uhl_PURPOSE_PREFIX', 'Order-ID:'); // this string will be prefixed to  the order ID in SEPA purpose Text
 */


__('Hello underscore only', 'mxp-sepa-qr-code-addon-for-woocommerce');


// Dummy image for dev purposes:
define('uhl_SAD_SMILEY', 'data:image/png;base64,');



/**
 *   add html snippet with qr-code after data table in thankyou page
 *   if the total amount is greater than 0
 */
function uhl_add_text_to_thankyoupage($order_id, $uhl_THANKYOU_PAGE)
{

	$order = wc_get_order($order_id);
	$order_data = $order->get_data();
	// do we need the user? if so: $user = $order->get_user();
	if (!empty($order->get_total()) && (float)$order->get_total() > 0) {
		echo '<p>' . esc_attr(__($uhl_THANKYOU_PAGE, 'mxp-sepa-qr-code-addon-for-woocommerce')) . '<br>';
		echo '<img class="uhl-bacs-qrcode" src="' . esc_attr(uhl_get_qrcode($order->get_total(), $order_id)) . '" alt="qr-code"></p>';
	}
}

// only executed when using bacs as payment:
add_action('woocommerce_thankyou_bacs', 'uhl_add_text_to_thankyoupage');
// use action 'woocommerce_thankyou' to execute with all payment methods


/**
 *   add  text snippet after the order table in email message
 *	if total amount is greater than 0
 *   since there is no specific hook for payment methods, we need to make sure that the code is
 *   only shown if bacs is used as payment method
 */

function uhl_email_after_order_table($order, $sent_to_admin, $plain_text, $email, $uhl_THANKYOU_EMAIL)
{
	// TODO: check whether spam filters are triggered by embedded image data,
	// if so replace image with a link to the  qr minipage ( site_url() . '?' . uhl_QUERY_PARAM . '=' . md5($order->get_total(). '_' . $order->get_id())
	// and activate uhl_USE_TRANSIENTS
	if (!empty($order->get_total()) && (float)$order->get_total() > 0 && $order->get_payment_method() == 'bacs' && ('on-hold' == $order->status || 'pending' == $order->status)) {
		echo '<p>' . esc_attr(__($uhl_THANKYOU_EMAIL, 'mxp-sepa-qr-code-addon-for-woocommerce')) . '<br> <img class="uhl-bacs-qrcode" src="' . esc_attr(uhl_get_qrcode($order->get_total(), $order->get_id())) . '"></p>';
	}
}

add_action('woocommerce_email_after_order_table', 'uhl_email_after_order_table', 10, 4);


/**
 *   testing qr generation by URL param ( value must be a valid md5 hash, otherwise you get the dummy 11-11 Code
 *   use 351436ef4b279e1811a6c68a2dd58b1b for testing ...
 */

function uhl_qrpage_from_hash($template)
{
	global $wp_query;

	if (!isset($wp_query->query[uhl_QUERY_PARAM]) || $wp_query->query[uhl_QUERY_PARAM] == '') {
		// stop processing if our param does not exist or is empty
		return $template;
	}

	$md5 = $wp_query->query[uhl_QUERY_PARAM];

	if (false === uhl_valid_md5($md5)) {
		$html = '<p>' .  __('This is not a valid md5 hash :-( ', 'mxp-sepa-qr-code-addon-for-woocommerce')  . '<br>' .  __('Generating uncached dummy QR for 11 Euro/OrderID 11', 'mxp-sepa-qr-code-addon-for-woocommerce') . '<br><img src="' . uhl_create_qrcode(11, 11) . '"></p>';
	} elseif (false === ($qrcode = uhl_get_transient_by_hash($md5))) {
		// valid md5, but nothing found, what a pity ...
		if ($md5 == "351436ef4b279e1811a6c68a2dd58b1b") {
			// this is the test md5 generated from 11_11 ... test for cache population
			uhl_set_transient(11, 11, uhl_SAD_SMILEY);
		}
		$html =  '<pre>QR-Code not found or expired. Doh!' . PHP_EOL . '</pre><img src="' . uhl_SAD_SMILEY . '" alt="Sad Smiley">';
	} else {
		$html = '<img class="uhl-bacs-qrcode" src="' . $qrcode . '" alt="QR Code">';
	}
	echo '<html><body><div>' . esc_attr($html) . ' </div></body></html>';
	// bail out without returning since we are done and do not want a complete WP page
	exit;
}
// TODO : check for a better place than template_redirect to hook into
add_action('template_redirect', 'uhl_qrpage_from_hash');


/**
 *   Introduce a custom query variable for QR retrieval and testing purposes
 */

function uhl_add_qv($vars)
{
	$vars[] = uhl_QUERY_PARAM;
	return $vars;
}
add_filter('query_vars', 'uhl_add_qv');


/*   Utility and Wrapper functions */

/**
 *   get/create QR code from amount and order id, using first bacs account or defaults
 *   first, fetch from cache, if possible
 *	store result into cache or transient storage for later retrieval
 *   create cache/transient ID from a md5 hash of amount and orderid
 */

function uhl_get_qrcode($amount, $orderid)
{

	if (false === ($qrcode = uhl_get_transient_by_data($amount, $orderid))) {
		// not found - so regenerate the code  and save as transient
		$qrcode = uhl_create_qrcode($amount, $orderid);
	}
	if (empty($qrcode)) {
		return false;
	}

	uhl_set_transient($amount, $orderid, $qrcode);
	return $qrcode;
}


/**
 * create a SEPA qr code based on first bacs account iban and bic , amount and id of order
 * store into transient for later retrieval in email link
 * SEPA QR-Code generation powered by https://github.com/fellwell5/bezahlcode/ (LGPL)
 * we use the local phpqrcode generation method to avoid external dependencies and data transmission
 */

function uhl_create_qrcode($amount, $orderid, $uhl_PURPOSE_PREFIX)
{

	$payloadtext = __($uhl_PURPOSE_PREFIX) . ' ' . $orderid;
	$bacs_accounts  = get_option('woocommerce_bacs_accounts');
	if (!empty($bacs_accounts[0])) {
		$iban =  $bacs_accounts[0]['iban'];
		$bic = $bacs_accounts[0]['bic'];
		$company = $bacs_accounts[0]['account_name'];
	}
	// fallbacks if empty:
	if (empty($iban)) {
		$iban = uhl_BACS_IBAN;
	}
	if (empty($bic)) {
		$bic = uhl_BACS_BIC;
	}
	if (empty($company)) {
		$company = uhl_BACS_COMPANY;
	}

	require_once(dirname(__FILE__) . "/vendor/bezahlcode/bezahlcode.class.php");

	$bezahlcode = new Bezahlcode($iban, $bic, $company, "phpqrcode", false);
	$bezahlcode->generatePayload($payloadtext, $amount);
	$base64 = $bezahlcode->generateBase64('png');
	if (empty($base64)) {
		return false;
	}
	return $base64;
}

// wrapper functions for caching or transients, depending on config

function uhl_get_transient_by_data($amount, $id)
{
	$transient = uhl_TRANSIENT_PREFIX . md5($amount . '_' . $id);
	if (defined('uhl_USE_TRANSIENTS')) {
		return get_transient($transient);
	}
	return wp_cache_get($transient);
}

function uhl_get_transient_by_hash($md5)
{
	$transient = uhl_TRANSIENT_PREFIX . $md5;
	if (defined('uhl_USE_TRANSIENTS')) {
		return get_transient($transient);
	}
	return wp_cache_get($transient);
}


function uhl_set_transient($amount, $id, $qrcode)
{
	$transient = uhl_TRANSIENT_PREFIX . md5($amount . '_' . $id);
	if (defined('uhl_USE_TRANSIENTS')) {
		$store = set_transient($transient, $qrcode, uhl_TRANSIENT_LIFETIME);
	} else {
		$store = wp_cache_set($transient, $qrcode, '', uhl_TRANSIENT_LIFETIME);
	}
	return $store;
}

// helper to validate  the string is syntactically correct md5

function uhl_valid_md5($md5 = '')
{
	return strlen($md5) == 32 && ctype_xdigit($md5);
}
